...
import { calculateTaskReward } from '../utils/xpCalculator'; // 🌟 Import utility
  ...
  return (
    <form onSubmit={handleSubmit} className="dark:bg-zinc-900 bg-white p-10 rounded-[2.5rem] shadow-2xl border dark:border-zinc-800 border-gray-100 mb-10 relative overflow-visible">
      ...
      <div className="relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6 mb-8">
          ...
          <div className="md:col-span-4">
            ...
            <DatePicker
              ...
              minDate={new Date()}
              aria-label="Task deadline picker" // 🌟 Accessibility
              autoComplete="off"
        ...
        <button disabled={isSubmitting} type="submit" className={`group w-full py-5 rounded-[1.5rem] font-black text-sm tracking-[0.2em] transition-all shadow-2xl flex items-center justify-center gap-6 ${isSubmitting ? 'bg-purple-800 text-white/50 cursor-not-allowed' : 'bg-purple-600 hover:bg-purple-700 text-white shadow-purple-500/30 active:scale-[0.98]'}`}>
          <span className="flex items-center gap-3">
            {isSubmitting ? '⚔️ COMMENCING...' : '⚔️ COMMENCE QUEST'}
          </span>
          {!isSubmitting && (
            <>
              ...
              <span className="bg-zinc-950/40 px-5 py-1.5 rounded-full text-purple-200 border border-white/10 group-hover:bg-zinc-950/60 transition-all font-black">
                +{calculateTaskReward(difficulty, priority, hardMode)} XP
              </span>
            </>
          )}
        </button>
